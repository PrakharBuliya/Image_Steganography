...
/**
 * WARNING: Prone to Buffer overflow.
 */
void secret_in(char * str, unsigned size) {
	char c;
	int i = 0;
	fflush(stdin);
	getchar();
...
int Encoding(char *argv_2, char *argv_3, char *argv_4)
{
	...
	printf("Enter your secret text and Press '/' To Stop : \t");
	secret_text(fp2);
	...
	printf("\nEnter The Magic String '#' '$' Only[max 10] : \t");

	secret_in(magic_str, 10);
